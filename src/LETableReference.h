 ...
 */
    ...
    LETableReference(const LETableReference &parent, LEErrorCode &success)
        : fFont(parent.fFont)
        , fTag(parent.fTag)
        , fParent(&parent)
        , fStart(parent.fStart)
        , fLength(parent.fLength)
    {
   ...
   * subset
   */
  LETableReference(const LETableReference &parent, size_t offset, size_t length, LEErrorCode &err)
      : fFont(parent.fFont)
      , fTag(parent.fTag)
      , fParent(&parent)
      , fStart((parent.fStart) + offset)
      , fLength(length)
    {
        if (LE_SUCCESS(err)) {
            if (isEmpty()) {
                ...
                if (fLength == LE_UINTPTR_MAX && fParent->fLength != LE_UINTPTR_MAX)
    ...
    size_t ptrToOffset(const void *atPtr, LEErrorCode &success) const
    {
        ...
        if ((atPtr < fStart) || (hasBounds() && (atPtr > fStart + fLength))) {
...
public:
    ...
    {
        if (isValid() && LE_SUCCESS(success) && fLength!=LE_UINTPTR_MAX && length!=LE_UINTPTR_MAX && offset!=LE_UINTPTR_MAX && (offset + length)>fLength) {
...
public:
    ...
    const T *getAlias(le_uint32 i, LEErrorCode &success) const { return ((const T *)(((const char*)getAlias()) + getOffsetFor(i, success))); }
    const T *getAliasRAW() const { LE_DEBUG_TR("getAliasRAW<>"); return (const T *)fStart; }
    const T &getObject(le_uint32 i, LEErrorCode &success) const { return *getAlias(i, success); }
    const T &operator()(le_uint32 i, LEErrorCode &success) const { return *getAlias(i, success); }
    ...
    LEReferenceToArrayOf(const LETableReference& parent, LEErrorCode & success)
...
public:
   ...
   */
    inline LEReferenceTo(const LETableReference &parent, LEErrorCode &success, const void* atPtr)
        : LETableReference(parent, parent.ptrToOffset(atPtr, success)
        , LE_UINTPTR_MAX, success)
    {
    ...
    inline LEReferenceTo(const le_uint8 *data, size_t length = LE_UINTPTR_MAX)
        : LETableReference(data, length)
    {
    }

    inline LEReferenceTo(const T *data, size_t length = LE_UINTPTR_MAX)
        : LETableReference((const le_uint8*)data, length)
    {
    }

    inline LEReferenceTo()
        : LETableReference(NULL)
    {
    }
